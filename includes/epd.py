...
class Epd(object):
  def __init__(self, spi, display):
    ...
    bus.write_byte_data(DEVICE_ADDRESS, 0x3a, 0x00) #SPI Passthrough
    #GPIO.setwarnings(False)
    #GPIO.setup(RST, GPIO.OUT)
    #GPIO.setup(DC, GPIO.OUT)
    #GPIO.setup(CS, GPIO.OUT)
    #GPIO.setup(BUSY, GPIO.IN)
    bus.write_byte_data(DEVICE_ADDRESS, 0x30, 0xfb) # RST_PIN = OUT, BUSY_PIN = IN
    bus.write_byte_data(DEVICE_ADDRESS, 0x31, 0xef) # DC_PI = OUT
  ...
  def _readBusy(self):
    for i in range(0, 400):
      if bus.read_byte_data(DEVICE_ADDRESS, 0x32) == 0xbf:
        # print 'Busy is Low'
        return
      time.sleep(0.01)

  def _writeCmd(self, command):
    bus.write_byte_data(DEVICE_ADDRESS, 0x35, 0x00) # DC LOW
  ...
  def _init(self):
    ...
    self.bus.write_byte_data(DEVICE_ADDRESS, 0x34, 0x04)         # Reset High
    time.sleep(0.1)
    self.bus.write_byte_data(DEVICE_ADDRESS, 0x34, 0x00)         # Reset Low
    time.sleep(0.01)
    self.bus.write_byte_data(DEVICE_ADDRESS, 0x34, 0x04)         # Reset High
